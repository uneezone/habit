...
### 📌 도메인 변경
1. gabia에서 도메인 구입 (원래 habit로 해야하지만 이미 사용중인 도메인이므로 hebit으로 대체)
2. AWS Route 53 호스팅 영역 생성 후 4개의 네임 서버를 가비아에서 설정
3. 이후 Route 53 에서 레코드 생성 후 레코드 이름과 퍼블릭 ipv4주소 입력
4. EC2 인스턴스에 연결하여 root 계정으로 전환 후 80 포트를 8080 포트로 포워딩 설정
5. EC2 보안그룹 -> 인바운드 규칙 설정 HTTP(80) 포트를 추가
